...
public class FitnessActivities
{
  ...
  {
    int i = TL.indexOfValue(paramString);
    if (i < 0) {}
    int j;
    do
    {
  ...
  {
    String str = (String)TL.get(paramInt);
    if (str == null) {
  ...
  {
    return paramDataPoint.getValue(Field.FIELD_ACTIVITY).asActivity();
  }
  
  @Deprecated
  public static void setValue(DataPoint paramDataPoint, String paramString)
  {
    paramDataPoint.getValue(Field.FIELD_ACTIVITY).setActivity(paramString);
  }
}
